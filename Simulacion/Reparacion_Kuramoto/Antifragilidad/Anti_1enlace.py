...
#A = (A > 0).astype(float)   #Si la red esta dañada arregla todos los enlaces para el analisis


# --- PARÁMETROS ---
#num_simulations = 1000
delta_B = 0.01
Betas = np.arange(0, 1+delta_B, delta_B)
enlace = 20
...
Funcionalidad, enlace = Simulacion(Betas, A, enlace)

Anti = Antifragilidad(Funcionalidad, Betas, n_points=4)
Data_F = np.array((Betas,Funcionalidad))
Data_Lambda = np.array((enlace, Anti))




np.save('Datos_Funcionalidad.npy',Data_F) ## GUARDAR DATOS
np.save('Datos_Lambda.npy',Data_Lambda) ## GUARDAR DATOS
